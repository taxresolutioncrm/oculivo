...
type Meta={id:string;scope_id:string;brand:string;title:string;signer_name:string;signer_email:string;original_path:string;signed_path:string|null;certificate_path:string|null;fields:Field[];token_hash:string;status:'pending'|'sent'|'viewed'|'signed'|'declined'|'voided';created_at:string;sent_at:string|null;opened_at:string|null;last_viewed_at?:string|null;view_count?:number;signed_at:string|null;declined_at:string|null;decline_reason:string|null;voided_at:string|null;expires_at:string;created_by:string;audit:any[];source_sha256?:string|null;signed_sha256?:string|null;certificate_sha256?:string|null}
...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(action==='create'){
   ...
   const id=crypto.randomUUID(),raw=secret(),path=`${scope.id}/original/${id}-${safe(String(b.source_filename||'document.pdf'))}`;const {error:up}=await admin.storage.from(BUCKET).upload(path,bytes,{contentType:'application/pdf',upsert:false});if(up)throw up;const at=now();const m:Meta={id,scope_id:scope.id,brand:'Oculivo',title,signer_name:signerName,signer_email:signerEmail,original_path:path,signed_path:null,certificate_path:null,fields,token_hash:await shaText(raw),status:'pending',created_at:at,sent_at:null,opened_at:null,last_viewed_at:null,view_count:0,signed_at:null,declined_at:null,decline_reason:null,voided_at:null,expires_at:new Date(Date.now()+14*86400000).toISOString(),created_by:user.id,audit:[{event:'created',at,actor:user.email}]};await writeMeta(admin,m);return json({ok:true,document:publicMeta(m,null),token:`${scope.id}.${id}.${raw}`,sign_url:`${String(b.origin||req.headers.get('origin')||'https://app.oculivo.com').replace(/\/$/,'')}/office-sign/${scope.id}.${id}.${raw}`})
  }

...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(action==='send_invite'){const m=await readMeta(admin,scope.id,String(b.document_id||''));if(!m)return json({error:'Document not found'},404);if(!['pending','sent','viewed'].includes(m.status))return json({error:'Document is not sendable'},409);const raw=String(b.token||'');if(!raw)return json({error:'Fresh signing token required'},400);const signUrl=`${String(b.origin||req.headers.get('origin')||'https://app.oculivo.com').replace(/\/$/,'')}/office-sign/${scope.id}.${m.id}.${raw}`;await sendInvite(admin,scope,m,signUrl);const at=now();m.status='sent';m.sent_at=at;m.audit.push({event:m.sent_at?'sent':'sent',at,actor:user.email});await writeMeta(admin,m);return json({ok:true,status:'sent'})}

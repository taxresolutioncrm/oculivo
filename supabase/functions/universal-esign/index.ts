...
type Progress={completed_field_ids:string[];completed_required:number;required_total:number;missing_pages:number[];updated_at:string}
type Meta={id:string;scope_id:string;brand:string;title:string;signer_name:string;signer_email:string;original_path:string;signed_path:string|null;certificate_path:string|null;fields:Field[];token_hash:string;status:'pending'|'sent'|'viewed'|'signed'|'declined'|'voided';created_at:string;sent_at:string|null;opened_at:string|null;last_viewed_at?:string|null;view_count?:number;progress?:Progress|null;signed_at:string|null;declined_at:string|null;decline_reason:string|null;voided_at:string|null;expires_at:string;created_by:string;audit:any[];source_sha256?:string|null;signed_sha256?:string|null;certificate_sha256?:string|null}
...
function publicMeta(m:Meta,url:string|null){return {id:m.id,title:m.title,brand:m.brand,signer_name:m.signer_name,signer_email:m.signer_email,fields:m.fields,status:m.status,expires_at:m.expires_at,sent_at:m.sent_at,opened_at:m.opened_at,last_viewed_at:m.last_viewed_at||m.opened_at,view_count:Number(m.view_count||0),signed_at:m.signed_at,declined_at:m.declined_at,decline_reason:m.decline_reason,certificate_path:m.certificate_path,progress:m.progress||null,file_url:url}}
...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(['load','progress','sign','decline'].includes(action)){
   ...
   if(action==='progress'){
    ...
    const changed=!prior||prior.completed_required!==next.completed_required||prior.required_total!==next.required_total||JSON.stringify(prior.missing_pages)!==JSON.stringify(next.missing_pages)
    m.progress=next
    if(changed)m.audit.push({event:'progress',at,completed_required:completedRequired,required_total:required.length,missing_pages:missingPages})
    await writeMeta(admin,m)
  ...
  if(action==='create'){
   ...
   if(bytes.length<5||String.fromCharCode(...bytes.slice(0,5))!=='%PDF-')return json({error:'Uploaded file must be a valid PDF'},400)
   if(!fields.some(f=>f.type==='signature'))return json({error:'At least one signature field is required'},400)
   const id=crypto.randomUUID(),raw=secret(),path=`${scope.id}/original/${id}-${safe(String(b.source_filename||'document.pdf'))}`;const {error:up}=await admin.storage.from(BUCKET).upload(path,bytes,{contentType:'application/pdf',upsert:false});if(up)throw up;const at=now();const m:Meta={id,scope_id:scope.id,brand:'Oculivo',title,signer_name:signerName,signer_email:signerEmail,original_path:path,signed_path:null,certificate_path:null,fields,token_hash:await shaText(raw),status:'pending',created_at:at,sent_at:null,opened_at:null,last_viewed_at:null,view_count:0,progress:null,signed_at:null,declined_at:null,decline_reason:null,voided_at:null,expires_at:new Date(Date.now()+14*86400000).toISOString(),created_by:user.id,audit:[{event:'created',at,actor:user.email}]};await writeMeta(admin,m);return json({ok:true,document:publicMeta(m,null),token:`${scope.id}.${id}.${raw}`,sign_url:`${String(b.origin||req.headers.get('origin')||'https://app.oculivo.com').replace(/\/$/,'')}/office-sign/${scope.id}.${id}.${raw}`})
  }
  if(action==='send_invite'){const m=await readMeta(admin,scope.id,String(b.document_id||''));if(!m)return json({error:'Document not found'},404);if(!['pending','sent','viewed'].includes(m.status))return json({error:'Document is not sendable'},409);const envelopeToken=String(b.token||'');const tokenParts=envelopeToken.split('.');if(tokenParts.length!==3||tokenParts[0]!==scope.id||tokenParts[1]!==m.id||m.token_hash!==await shaText(tokenParts[2]))return json({error:'Fresh signing token required'},400);const signUrl=`${String(b.origin||req.headers.get('origin')||'https://app.oculivo.com').replace(/\/$/,'')}/office-sign/${envelopeToken}`;await sendInvite(admin,scope,m,signUrl);const at=now(),wasSent=Boolean(m.sent_at);m.status='sent';m.sent_at=at;m.audit.push({event:wasSent?'resent':'sent',at,actor:user.email});await writeMeta(admin,m);return json({ok:true,status:'sent'})}
  if(action==='resend_invite'){const m=await readMeta(admin,scope.id,String(b.document_id||''));if(!m)return json({error:'Document not found'},404);if(!['sent','viewed'].includes(m.status))return json({error:'Document is not resendable'},409);const raw=secret(),envelopeToken=`${scope.id}.${m.id}.${raw}`,signUrl=`${String(b.origin||req.headers.get('origin')||'https://app.oculivo.com').replace(/\/$/,'')}/office-sign/${envelopeToken}`;m.token_hash=await shaText(raw);await sendInvite(admin,scope,m,signUrl);const at=now();m.status='sent';m.sent_at=at;m.audit.push({event:'resent',at,actor:user.email});await writeMeta(admin,m);return json({ok:true,status:'sent'})}
  if(action==='list'){const {data,error}=await admin.storage.from(BUCKET).list(`${scope.id}/meta`,{limit:200,sortBy:{column:'created_at',order:'desc'}});if(error)throw error;const docs=[];for(const f of data||[]){if(!f.name.endsWith('.json'))continue;const m=await readMeta(admin,scope.id,f.name.replace(/\.json$/,''));if(m)docs.push({...publicMeta(m,null),audit:m.audit})}return json({ok:true,documents:docs})}

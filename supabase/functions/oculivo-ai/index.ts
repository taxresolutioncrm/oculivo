...
Deno.serve(async(req:Request)=>{
 ...
 const org=String(b.organization_id||"").trim(); if(!org)return json({error:"organization_id required"},400);
 const message=String(b.message||"").trim().slice(0,4000); if(!message)return json({error:"message required"},400);

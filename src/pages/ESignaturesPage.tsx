...
type Progress={completed_required:number;required_total:number;missing_pages:number[];updated_at:string} 
type Doc={id:string;title:string;signer_name:string;signer_email:string;status:string;sent_at?:string|null;opened_at?:string|null;last_viewed_at?:string|null;view_count?:number;signed_at?:string|null;declined_at?:string|null;decline_reason?:string|null;certificate_path?:string|null;progress?:Progress|null;audit?:AuditEvent[]}
 ...
 return <main style={{padding:24,maxWidth:1200,margin:'0 auto',fontFamily:'Inter,system-ui,sans-serif',color:'#172033'}}>
  ...
  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,320px),1fr))',gap:18,alignItems:'start'}}>
   ...
   <section style={{border:'1px solid #e2e8f0',borderRadius:14,padding:18,background:'#fff'}}>
    ...
    {!docs.length?<p style={{color:'#64748b'}}>{lang==='es'?'Aún no hay sobres.':'No envelopes yet.'}</p>:<div style={{display:'grid',gap:9}}>{docs.map(d=><div key={d.id} style={{border:'1px solid #e2e8f0',borderRadius:10,padding:12}}><div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><strong>{d.title}</strong><span style={{color:'#64748b'}}>{d.signer_email}</span><span style={{marginLeft:'auto',fontSize:11,fontWeight:900,textTransform:'uppercase'}}>{status(d.status)}</span></div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:5,marginTop:8,fontSize:11,color:'#64748b'}}><span>{lang==='es'?'Enviado':'Sent'}: <b>{fmt(d.sent_at)}</b></span><span>{lang==='es'?'Abierto':'Opened'}: <b>{fmt(d.opened_at)}</b></span><span>{lang==='es'?'Última vista':'Last viewed'}: <b>{fmt(d.last_viewed_at)}</b></span><span>{lang==='es'?'Vistas':'Views'}: <b>{d.view_count||0}</b></span><span>{lang==='es'?'Firmado':'Signed'}: <b>{fmt(d.signed_at)}</b></span><span>{lang==='es'?'Progreso':'Progress'}: <b>{d.progress?`${d.progress.completed_required}/${d.progress.required_total}`:'—'}</b></span><span>{lang==='es'?'Páginas pendientes':'Missing pages'}: <b>{d.progress?.missing_pages?.length?d.progress.missing_pages.join(', '):'—'}</b></span><span>{lang==='es'?'Eventos':'Events'}: <b>{d.audit?.length||0}</b></span></div>{reminderDue(d)&&<div style={{marginTop:7,padding:'6px 8px',borderRadius:7,background:'#fff7ed',color:'#9a3412',fontSize:11,fontWeight:800}}>{lang==='es'?'Recordatorio pendiente: enviado hace más de 24 horas':'Reminder due: sent more than 24 hours ago'}</div>}{d.decline_reason&&<div style={{marginTop:6,color:'#991b1b',fontSize:12}}>{lang==='es'?'Rechazado':'Declined'}: {d.decline_reason}</div>}<div style={{display:'flex',gap:7,marginTop:9,flexWrap:'wrap'}}><button onClick={()=>openDoc(d.id)} style={buttonStyle}>{lang==='es'?'Abrir':'Open'}</button>{d.status==='signed'&&d.certificate_path&&<button onClick={()=>openDoc(d.id,'certificate')} style={buttonStyle}>{lang==='es'?'Certificado':'Certificate'}</button>}{['sent','viewed'].includes(d.status)&&<button disabled={busy} onClick={()=>void resend(d.id)} style={buttonStyle}>{lang==='es'?'Recordatorio / reenviar':'Reminder / resend'}</button>}{!['signed','voided','declined'].includes(d.status)&&<button disabled={busy} onClick={()=>voidDoc(d.id)} style={buttonStyle}>{lang==='es'?'Anular':'Void'}</button>}</div></div>)}</div>}
   </section>
  </div>
 </main>
}

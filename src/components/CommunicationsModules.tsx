...
export function PhoneOps({session,lang}:{session:Session;lang:Lang}){
 const {orgId,canCommunicate}=useOrgAccess(session),[threads,setThreads]=useState<Row[]>([]),[threadId,setThreadId]=useState(''),[callHistory,setCallHistory]=useState<Row[]>([]),[status,setStatus]=useState<'idle'|'preparing'|'ringing'|'connected'|'ended'|'error'>('idle'),[destination,setDestination]=useState(''),[dialNumber,setDialNumber]=useState(''),[error,setError]=useState(''),[faxFile,setFaxFile]=useState<File|null>(null),[faxBusy,setFaxBusy]=useState(false),[faxMessage,setFaxMessage]=useState('')
 ...
 async function load(selectId?:string){if(!orgId){setThreads([]);setThreadId('');return}const r=await supabase.from('communication_threads').select('*').eq('organization_id',orgId).eq('channel','phone').order('last_message_at',{ascending:false,nullsFirst:false}).limit(200);if(r.error){setError(r.error.message);return}setThreads(r.data||[]);const target=selectId||threadId;if(target&&(r.data||[]).some(x=>String(x.id)===target))setThreadId(target);else if(r.data?.[0]?.id)setThreadId(String(r.data[0].id));else setThreadId('')}
 async function loadCallHistory(id=threadId){if(!orgId||!id){setCallHistory([]);return}const r=await supabase.from('communication_messages').select('id,direction,body,sender_name,sender_address,created_at').eq('organization_id',orgId).eq('thread_id',id).order('created_at',{ascending:false}).limit(100);if(r.error){setError(r.error.message);return}setCallHistory((r.data||[]).filter(x=>String(x.body||'').toLowerCase().includes('call')))}
 useEffect(()=>{setThreadId('');setCallHistory([]);setDestination('');setDialNumber('');void load()},[orgId])
 useEffect(()=>{void loadCallHistory(threadId)},[threadId,orgId])
 ...
 async function finish(action:'complete'|'failed'){
  ...
  await report(action)
  await cleanup()
  await load(threadId||undefined)
  await loadCallHistory(threadId||undefined)
 }
 async function start(){
  if(!canCommunicate||(!threadId&&!dialNumber.trim()))return
  terminalRef.current=false;messageIdRef.current='';setStatus('preparing');setError('')
  const body:any={action:'prepare'}
  if(threadId)body.thread_id=threadId
  else{body.organization_id=orgId;body.to=dialNumber.trim()}
  const prep=await supabase.functions.invoke('phone-session',{body})
  ...
  try{
   const resolvedThread=String(prep.data.thread_id||threadId||'')
   if(resolvedThread){setThreadId(resolvedThread);await load(resolvedThread)}
   messageIdRef.current=String(prep.data.message_id||'');setDestination(String(prep.data.destination||''));setDialNumber('')
 ...
 return <section><audio ref={audioRef} autoPlay playsInline/><div className="wf-head"><div><h2>{lang==='es'?'Teléfono':'Phone'}</h2><p>{lang==='es'?'Llamadas del consultorio desde el navegador':'Practice calls from your browser'}</p></div><button className="refresh-button" onClick={()=>void load()}><RefreshCw size={14}/>{lang==='es'?'Actualizar':'Refresh'}</button></div>{error&&<div className="wf-alert">{error}</div>}<div className="phone-workspace"><aside className="inbox-thread-list">{!threads.length&&<div className="inbox-empty"><Phone size={20}/><strong>{lang==='es'?'Sin conversaciones telefónicas':'No phone conversations'}</strong><p>{lang==='es'?'Marca un número E.164 para iniciar la primera llamada.':'Dial an E.164 number to start the first call.'}</p></div>}{threads.map(x=><button key={x.id} className={threadId===String(x.id)?'active':''} onClick={()=>{setThreadId(String(x.id));setDestination(String(x.phone_number||''));setDialNumber('');setStatus('idle');setError('')}}><strong>{x.subject||x.phone_number||(lang==='es'?'Conversación telefónica':'Phone conversation')}</strong><span>{x.phone_number||''}</span></button>)}</aside><div className="phone-dialer-card"><div className={'phone-orb '+status}><Phone size={28}/></div><span className="date-kicker">{localized(status,lang).toUpperCase()}</span><h2>{active?.phone_number||destination||dialNumber||(lang==='es'?'Nueva llamada':'New call')}</h2><p>{status==='connected'?(lang==='es'?'Llamada conectada':'Call connected'):status==='ringing'?(lang==='es'?'Llamando…':'Ringing…'):status==='preparing'?(lang==='es'?'Preparando sesión segura…':'Preparing secure session…'):(lang==='es'?'Usa el micrófono de este navegador para llamar desde la identidad telefónica del consultorio.':'Uses this browser microphone and the practice phone identity.')}</p>{!inCall&&canCommunicate&&<div className="wf-field"><span>{lang==='es'?'Número E.164 para nueva llamada':'E.164 number for new call'}</span><input type="tel" placeholder="+15551234567" value={dialNumber} onChange={e=>{setDialNumber(e.target.value);if(e.target.value.trim())setThreadId('')}}/></div>}{!inCall?<button className="wf-primary" disabled={!canCommunicate||(!threadId&&!dialNumber.trim())} onClick={()=>void start()}><Phone size={16}/>{status==='ended'||status==='error'?(lang==='es'?'Llamar otra vez':'Call again'):(lang==='es'?'Iniciar llamada':'Start call')}</button>:<button className="wf-danger" disabled={status==='preparing'} onClick={()=>void hangup()}><PhoneOff size={16}/>{lang==='es'?'Colgar':'Hang up'}</button>}<div className="phone-mic-note"><Mic size={13}/>{!canCommunicate?(lang==='es'?'Tu rol tiene acceso de solo lectura.':'Your role has read-only access.'):(lang==='es'?'El navegador pedirá permiso para usar el micrófono.':'Your browser will ask for microphone permission.')}</div>
<div className="phone-history-card"><div><strong>{lang==='es'?'Historial de llamadas':'Call history'}</strong><span>{lang==='es'?'Últimos 100 registros del hilo seleccionado':'Latest 100 records for the selected phone thread'}</span></div>{!threadId?<p>{lang==='es'?'Selecciona una conversación telefónica para ver su historial.':'Select a phone conversation to view its history.'}</p>:!callHistory.length?<p>{lang==='es'?'Todavía no hay llamadas registradas en este hilo.':'No calls are recorded in this thread yet.'}</p>:<div className="wf-list">{callHistory.map(x=><article className="wf-row" key={x.id}><div className="wf-grow"><h3>{localized(x.direction,lang)|| (lang==='es'?'Llamada':'Call')}</h3><p>{t(x.body)}</p></div><span>{new Date(x.created_at).toLocaleString()}</span></article>)}</div>}</div><div className="fax-send-card"><div><FileText size={16}/><strong>{lang==='es'?'Enviar fax':'Send fax'}</strong></div><p>{lang==='es'?'Selecciona una conversación telefónica existente y un PDF privado del consultorio.':'Select an existing phone conversation and a private practice PDF.'}</p><input type="file" accept="application/pdf,.pdf" disabled={!threadId||!canCommunicate||faxBusy} onChange={e=>setFaxFile(e.target.files?.[0]||null)}/><button className="wf-primary" disabled={!threadId||!canCommunicate||!faxFile||faxBusy} onClick={()=>void sendFax()}><FileText size={15}/>{faxBusy?(lang==='es'?'Enviando fax…':'Sending fax…'):(lang==='es'?'Enviar fax':'Send fax')}</button>{faxMessage&&<div className="wf-alert">{faxMessage}</div>}</div></div></div></section>
}

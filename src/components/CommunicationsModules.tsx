...
export function PhoneOps({session,lang}:{session:Session;lang:Lang}){
 const {orgId,canCommunicate}=useOrgAccess(session),[threads,setThreads]=useState<Row[]>([]),[threadId,setThreadId]=useState(''),[status,setStatus]=useState<'idle'|'preparing'|'ringing'|'connected'|'ended'|'error'>('idle'),[destination,setDestination]=useState(''),[dialNumber,setDialNumber]=useState(''),[error,setError]=useState(''),[faxFile,setFaxFile]=useState<File|null>(null),[faxBusy,setFaxBusy]=useState(false),[faxMessage,setFaxMessage]=useState('')
 ...
 useEffect(()=>{setThreadId('');setDestination('');setDialNumber('');void load()},[orgId])

...
const inboundSmsFn=read('supabase/functions/receive-sms/index.ts')
const inboundEmailFn=read('supabase/functions/receive-email/index.ts')
...
const integrityMigration=read('supabase/migrations/20260918110000_clinical_schedule_communications_integrity.sql')
...
const requiredFunctions=['oculivo-ai','oculivo-support-api','phone-session','send-email','send-sms','receive-email','receive-sms','send-fax','universal-esign']
...
check([aiFn,supportFn,phoneFn,faxFn,emailFn,smsFn,esignFn,inboundSmsFn,inboundEmailFn].every(x=>x.includes('@supabase/supabase-js@2.116.0')),'All Oculivo Edge Functions pin the patched Supabase JS client')
...
check(config.includes('[functions.receive-sms]\nverify_jwt = false')&&config.includes('[functions.receive-email]\nverify_jwt = false'),'Inbound provider webhooks explicitly allow signed external callback traffic')
...
check(inboundSmsFn.includes('OCULIVO_INBOUND_SMS_SECRET')&&inboundSmsFn.includes('Invalid webhook signature')&&inboundSmsFn.includes('direction:"inbound"'),'Inbound SMS verifies a practice-scoped HMAC and stores unread CRM messages')
check(inboundEmailFn.includes('OCULIVO_INBOUND_EMAIL_SECRET')&&inboundEmailFn.includes('ExtractedMarkdownMessage')&&inboundEmailFn.includes('direction:"inbound"'),'Inbound Brevo email parsing verifies a practice-scoped HMAC and stores unread CRM messages')
check(inboundEmailFn.includes('provider_message_id')&&inboundEmailFn.includes('item?.MessageId'),'Inbound email is idempotent by Brevo MessageId')
check(inboundSmsFn.includes('provider_message_id')&&inboundSmsFn.includes('MessageSid')&&inboundSmsFn.includes('communication_endpoints'),'Inbound SMS is idempotent and bound to a configured practice destination number')
check(emailFn.includes('provider_message_id')&&smsFn.includes('provider_message_id'),'Outbound email and SMS retain provider message identifiers in the CRM audit trail')
...
check(ops.includes("from('optical_orders').update(payload)")&&ops.includes("from('optical_inventory').update(payload)")&&ops.includes('setOrderStatus'),'Optical orders and inventory support editing and order-status progression')
check(ops.includes("from('insurance_claims').update(payload)")&&ops.includes('editClaim'),'Insurance claims support editing instead of create-only workflow')
check(ops.includes('weekHours')&&ops.includes('todayHours')&&ops.includes('hours(r).toFixed(2)'),'Timeclock shows worked-hour totals alongside punch history')
check(comms.includes('New conversation')&&comms.includes('startConversation')&&comms.includes("organization_id:orgId,to:newTo.trim()"),'Inbox can initiate new email and SMS conversations')
check(emailFn.includes('organization_id?:string')&&emailFn.includes('Could not create email conversation')&&emailFn.includes('thread_id:thread.id'),'Email backend can create a practice-scoped outbound conversation')
...
check(integrityMigration.includes('clinical_records_signed_immutable')&&integrityMigration.includes('Signed clinical records are immutable'),'Signed clinical records are protected at the database layer')
check(integrityMigration.includes('appointments_integrity_guard')&&integrityMigration.includes('pg_advisory_xact_lock'),'Provider appointment overlap prevention is serialized at the database layer')
check(integrityMigration.includes('appointments_no_delete')&&integrityMigration.includes('Appointments must be cancelled, not deleted'),'Appointment audit history is protected from hard deletes')
check(ops.includes("update({status:'cancelled'})")&&!ops.includes("from('appointments').delete()"),'Schedule cancels appointments instead of deleting history')
check(ops.includes("scheduleFilter==='upcoming'||scheduleFilter==='today'"),'Upcoming and today schedule views are ordered chronologically')
...
check(inboundEmailFn.includes('communication_endpoints')&&inboundEmailFn.includes('item?.To')&&inboundEmailFn.includes('fallbackEmail'),'Inbound email maps the provider destination to the signed practice with endpoint-table or configured-sender fallback')
check(inboundSmsFn.includes('communication_endpoints')&&inboundSmsFn.includes('fallbackSms')&&inboundSmsFn.includes('OCULIVO_SMS_FROM_NUMBER'),'Inbound SMS maps the destination number to the signed practice with endpoint-table or configured-number fallback')
...
check(app.includes('oculivo-shell-unread-')&&app.includes("path==='/inbox'&&unreadCount>0")&&app.includes(".eq('is_read',false)"),'Shell shows realtime unread communication badges from practice-scoped inbound messages')

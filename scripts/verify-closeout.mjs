...
check(config.includes('[functions.universal-esign]')&&config.includes('verify_jwt = false'),'Public signer function is configured for token-scoped public actions')
...
check(emailFn.includes('BREVO_API_KEY')&&emailFn.includes('OCULIVO_FROM_EMAIL')&&emailFn.includes("communication_messages").toString(),'Email provider configuration and CRM audit logging are present')
...
check(smsFn.includes('OCULIVO_SMS_FROM_NUMBER')&&smsFn.includes('SIGNALWIRE_AUTH_TOKEN')&&smsFn.includes("communication_messages").toString(),'SMS provider configuration and CRM audit logging are present')
